...
*Draft v20 — supersedes v19. Adds §7a: Local vs. Remote — names the zero-internet operating mode explicitly (renamed from an earlier working title to match the Local/Remote framing), and identifies that the "6 users" hard cap is a Tailscale free-tier pricing artifact, not an architectural limit; Local Mode has no real structural user-count limit at any scale this project reaches (the GPU queue is the only shared constraint, and it degrades gracefully rather than failing). Records Headscale as a self-hosted alternative that removes the Tailscale cap entirely — not yet adopted. Re-justifies §7's 6-seat number as a deliberate UX/data-sharing/security decision, independent of whichever coordination layer provides Remote access. Also documents a Local-Mode-only fallback for web archiving. Compiled from working discussion, [date: 2026-09-16].*
