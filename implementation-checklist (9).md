...
1. **Build a disposable test vault**
   - [ ] Create a new, throwaway GitJournal vault (not the real one)
   - [ ] Add 2–3 markdown notes with YAML frontmatter
   - [ ] Insert one image directly (GitJournal's native image support), confirm it's a local file in the repo, not a hotlink
...
5. **Repeat via GitJournal on Android**
   - [ ] Clone the same test repo using GitJournal on a phone/tablet
   - [ ] Repeat the integrity checks from step 4 on this restored copy
   - [ ] Note any difference in behavior vs. the desktop git restore — this is the actual path most household members would use

6. **Record the result**
   - [ ] Date tested:
   - [ ] GitHub remote restore: pass / fail (notes: )
   - [ ] Local-only remote restore: pass / fail (notes: )
   - [ ] GitJournal (Android) restore: pass / fail (notes: )
...
- [ ] Decide how less-technical household members learn the two-app model (GitJournal for notes, Open-WebUI for AI chat) without needing to understand the architecture behind it
...
*Companion to spec Draft v20. Item 1's steps and item 13 updated to reference GitJournal rather than Obsidian/GitSync, matching the Personal Layer editor decision. Update the spec's Executive Summary once items are completed here.*
