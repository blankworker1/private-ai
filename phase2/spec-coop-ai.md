...
5. **Signed git push** — the designated board member executes the push to the self-hosted Forgejo instance, with the commit message referencing the Loomio poll's ID/URL. Git-level enforcement is identity-verification only (SSH-signed via the same hardware token used for Loomio/Tailscale auth) — no separate approval-count gate, since Loomio's poll is already the governance decision.
