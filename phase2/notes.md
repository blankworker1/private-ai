...
Follows directly from #1: hardware sizing for the node PC needs headroom for five always-on services sharing one machine's RAM/disk (Ollama, Open-WebUI, Redis, Forgejo, Loomio), not the four currently implied by §6. Loomio itself is lightweight, but it's still another persistent process on the same box.
