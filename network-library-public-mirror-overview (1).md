...
**Governance (Archiver Protocol v2):** hardware-security-token identity → draft in Notes/Channels → formal Loomio proposal and vote → recorded Outcome → signed git push to self-hosted Gitea, referencing the Loomio decision.
...
Self-hosted Gitea on a small, fixed-price VPS (~€4–10/month, Hetzner-class) — no cloud GPU, no third-party git platform. Compute needs are minimal without an AI workload; **storage and bandwidth are the real variables**, sized against actual archive content once known (§6).

Reused, not rebuilt: Gitea (second instance, same tool as the private Archiver Gitea), `oikb` (likely reusable for the relay box's push job), Gitea's own file browsing/search as the public front end's base.

Genuinely new: the VPS, the relay box, a thin front-end skin over Gitea's browse/search/download, standard Cloudflare protection (Tunnel + bot/DDoS).
...
2. Clean, stable, fetchable URLs per document (Gitea's raw file/repo-browse links already suit this).
