...
- **Web clipping — no longer a Personal Layer app requirement.** Previously handled by the Mobile Webviewer plugin; now a decoupled service (ArchiveBox/Wallabag) running on the Brain PC, fed by the phone's normal share sheet rather than tied to whichever note editor is installed — see §5 and `data-template.md` §4a for the full mechanism. This also means the choice of markdown editor no longer needs to include a built-in clipper as a requirement.
...
Pointed at the same vault via git: **Obsidian (desktop)**, **Obsidian Git plugin** (or CLI), **Save as Local Images**. Web clipping no longer needed here either — same decoupled archiving service as §2, device-agnostic. Fully functional, not required for daily use.
...
*Draft v18 — supersedes v17. Decouples web clipping from the Personal Layer editor entirely: removes the Mobile Webviewer plugin requirement (§2/§3), adds a web archiving service (ArchiveBox/Wallabag) to the Shared AI Brain (§5), fed by any device's share sheet rather than an in-app plugin. This also removes clipping as a factor in the Personal Layer editor choice — see `data-template.md` §4a for the full mechanism. Compiled from working discussion, [date: 2026-09-15].*
